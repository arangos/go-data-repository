...
// Client maps to the 'client' table.
type Client struct {
	CustomerCode     string    `gorm:"primaryKey;column:CUSTOMER_CODE;size:100;not null;uniqueIndex" json:"customerCode"`
	...
	ActiveCampaignID int64     `gorm:"column:ACTIVE_CAMPAIGN_ID" json:"activeCampaignId,omitempty"`
	DealOwner        string    `gorm:"column:DEAL_OWNER" json:"dealOwner,omitempty"`
	DealOwnerName    string    `gorm:"column:DEAL_OWNER_NAME" json:"dealOwnerName,omitempty"`
}

// BeforeCreate sets audit fields before insert
func (c *Client) BeforeCreate(tx *gorm.DB) (err error) {
	c.CreatedDate = time.Now()
	// set CreatedBy via context or util (if available)
	return
}

// BeforeUpdate sets audit fields before update
func (c *Client) BeforeUpdate(tx *gorm.DB) (err error) {
	c.LastModifiedDate = time.Now()
	return
}
